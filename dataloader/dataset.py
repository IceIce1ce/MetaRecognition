...
class VideoDataset(Dataset):
    def __init__(self, dataset='ucf101', split='train', clip_len=16, preprocess=False):
        if not os.path.exists('output'):
            os.mkdir('output')
        if dataset == 'ucf101':
            self.root_dir = 'dataset/UCF-101'
            self.output_dir = 'output/ucf101'
        else:
        ...
        self.crop_size = 112
        # Preprocess and split dataset before training
        ...
        if dataset == "ucf101":
            if not os.path.exists('dataloader/ucf_labels.txt'):
                with open('dataloader/ucf_labels.txt', 'w') as f:
    ...
    def preprocess(self):
        ...
        for file in os.listdir(self.root_dir):
            ...
            # split 60% train, 20% val and 20% test
    ...
    def randomflip(self, buffer):
        if np.random.random() < 0.5: # flip with prob = 50%
