# https://github.com/Finspire13/pytorch-i3d-feature-extraction/blob/master/pytorch_i3d.py
# (simple implementation: https://github.com/rimchang/kinetics-i3d-Pytorch/blob/master/model/I3D_Pytorch.py)
import torch
...
class InceptionI3d(nn.Module):
    VALID_ENDPOINTS = ('Conv3d_1a_7x7', 'MaxPool3d_2a_3x3', 'Conv3d_2b_1x1', 'Conv3d_2c_3x3', 'MaxPool3d_3a_3x3', 'Mixed_3b', 'Mixed_3c', 'MaxPool3d_4a_3x3', 'Mixed_4b',
                       'Mixed_4c', 'Mixed_4d', 'Mixed_4e', 'Mixed_4f', 'MaxPool3d_5a_2x2', 'Mixed_5b', 'Mixed_5c', 'Logits', 'Predictions')
    def __init__(self, num_classes=400, spatial_squeeze=True, final_endpoint='Logits', name='inception_i3d', in_channels=3, dropout_keep_prob=0.5):
        ...
        if self._final_endpoint == end_point:
            return
        end_point = 'Logits'
        self.avg_pool = nn.AvgPool3d(kernel_size=[2, 4, 4], stride=(1, 1, 1)) # if (224, 224) -> kernel_size=[2, 7, 7]
    ...
    def forward(self, x):
        ...
        x = self.logits(self.dropout(self.avg_pool(x))) # [1, 101, 1, 4, 4]
        if self._spatial_squeeze:
            logits = x.squeeze(3).squeeze(3).squeeze(2) # x.squeeze(3).squeeze(3)
        return logits
