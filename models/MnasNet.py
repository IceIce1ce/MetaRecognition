...
def Conv_3x3(inp, oup, stride):
    return nn.Sequential(nn.Conv3d(inp, oup, 3, stride, 1, bias=False), nn.BatchNorm3d(oup), nn.ReLU6(inplace=True))
...
def SepConv_3x3(inp, oup):
    return nn.Sequential(nn.Conv3d(inp, inp, 3, 1, 1, groups=inp, bias=False), nn.BatchNorm3d(inp), nn.ReLU6(inplace=True),
                         nn.Conv3d(inp, oup, 1, 1, 0, bias=False), nn.BatchNorm3d(oup))
...
class InvertedResidual(nn.Module):
    def __init__(self, inp, oup, stride, expand_ratio, kernel):
        ...
        self.conv = nn.Sequential(nn.Conv3d(inp, inp * expand_ratio, 1, 1, 0, bias=False), nn.BatchNorm3d(inp * expand_ratio), nn.ReLU6(inplace=True),
                                  nn.Conv3d(inp * expand_ratio, inp * expand_ratio, kernel, stride, kernel // 2, groups=inp * expand_ratio, bias=False),
                                  nn.BatchNorm3d(inp * expand_ratio),
                                  nn.ReLU6(inplace=True),
                                  nn.Conv3d(inp * expand_ratio, oup, 1, 1, 0, bias=False), nn.BatchNorm3d(oup))
...
class MnasNet(nn.Module):
    def __init__(self, num_classes=1000, input_size=224, width_mult=1.):
        ...
        # t, c, n, s, k
        self.interverted_residual_setting = [[3, 24,  3, 2, 3], # -> 56x56
                                             [3, 40,  3, 2, 5],  # -> 28x28
                                             [6, 80,  3, 2, 5],  # -> 14x14
                                             [6, 96,  2, 1, 3],  # -> 14x14
                                             [6, 192, 4, 2, 5],  # -> 7x7
