# https://github.com/MRzzm/action-recognition-models-pytorch/blob/master/3DCNN/P3D/P3D.py
...
class P3D_Block(nn.Module):
    def __init__(self, blockType, inplanes, planes, stride=1):
        ...
        if self.blockType == 'A':
            self.conv2D = nn.Conv3d(planes, planes, kernel_size=(1, 3, 3), stride=(1, stride, stride), padding=(0, 1, 1), bias=False)
            self.conv1D = nn.Conv3d(planes, planes, kernel_size=(3, 1, 1), stride=(stride, 1, 1), padding=(1, 0, 0), bias=False)
        elif self.blockType == 'B':
            self.conv2D = nn.Conv3d(planes, planes, kernel_size=(1, 3, 3), stride=stride, padding=(0, 1, 1), bias=False)
            self.conv1D = nn.Conv3d(planes, planes, kernel_size=(3, 1, 1), stride=stride, padding=(1, 0, 0), bias=False)
        else:
            self.conv2D = nn.Conv3d(planes, planes, kernel_size=(1, 3, 3), stride=stride, padding=(0, 1, 1), bias=False)
            self.conv1D = nn.Conv3d(planes, planes, kernel_size=(3, 1, 1), stride=1, padding=(1, 0, 0), bias=False)
        ...
        if self.stride != 1 or inplanes != planes * self.expansion:
            self.downsample = nn.Sequential(nn.Conv3d(inplanes, planes * self.expansion, kernel_size=1, stride=stride, bias=False),
...
class P3D(nn.Module):
    def __init__(self, num_classes):
        ...
        self.average_pool = nn.AvgPool3d((1, 2, 2)) # (1, 3, 3) for size 160 x 160
...
if __name__ == "__main__":
    X = torch.rand(1, 3, 16, 112, 112) # (1, 3, 16, 160, 160)
    model = P3D(num_classes=101)
    print(model)
