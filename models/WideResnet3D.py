# https://github.com/yangbang18/video-classification-3d-cnn/blob/master/models/resnet.py
import torch
...
class WideResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(x.size(0), -1)
        if self.last_fc:
            x = self.fc(x) # change to false to use the model as feature extractor
        return x

def WideResnet3D_50(**kwargs):
    return WideResNet(WideBottleneck, [3, 4, 6, 3], **kwargs)

def WideResnet3D_101(**kwargs):
    return WideResNet(WideBottleneck, [3, 4, 23, 3], **kwargs)

def WideResnet3D_152(**kwargs):
    return WideResNet(WideBottleneck, [3, 8, 36, 3], **kwargs)

def WideResnet3D_200(**kwargs):
    return WideResNet(WideBottleneck, [3, 24, 36, 3], **kwargs)

if __name__ == "__main__":
    X = torch.rand(1, 3, 16, 112, 112)
    model = WideResnet3D_50(sample_size=112, sample_duration=16, num_classes=101)
    print(model)
